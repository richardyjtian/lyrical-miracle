...
class PlayPage extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      displayTrackName: "Pending connection",
      artistName: "Artist Name",
      displayArtistName: "",
      imageURL:
        "https://images-na.ssl-images-amazon.com/images/I/71IywUSFmTL._AC_UL600_SR600,600_.jpg",
      songLyrics:
        "After authenticating with Spotify, </br> connect to this web app through </br> any Spotify app on your PC or phone </br> as you would with a smart speaker </br> (In Spotify, click the desktop icon in </br> the bottom right corner next to the </br> heart icon)",
      isPlay: false,
    };
  }
  ...
  handleLoadSuccess() {
    ...
    player.addListener("player_state_changed", (state) => {
      ...
      if (track_name !== cur_track_name) {
        ...
        var genius_json_rsp = this.httpGetGeniusSearch(
          track_name,
          cur_artist_name
        );
        ...
        if (typeof song !== "undefined") {
          ...
          let lyrics = parser
            ...
            .getElementsByClassName("lyrics")[0].innerHTML;
          const a_open_regex = /<a([\s\S]*?)>/g;
          const a_close_regex = /<\/a>/g;
          lyrics = lyrics
            .replaceAll(a_open_regex, "")
            .replaceAll(a_close_regex, "");
  ...
  httpGetGeniusLyrics(url) {
    const proxyurl = "https://silly-wan-cors.herokuapp.com/";
    url = proxyurl + url;
    const xmlHttp = new XMLHttpRequest();
    xmlHttp.open("GET", url, false); // false for synchronous request
    xmlHttp.send();
      ...
      <div className="PlayPage">
        ...
        <div className="LyricBox">
          <LyricBox
            ...
            title={this.state.displayTrackName}
            artist={this.state.displayArtistName}
          />
        </div>
      </div>
    );
